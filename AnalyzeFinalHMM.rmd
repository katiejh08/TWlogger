...
*This creates one file with states and raw acc. Saves as data_states.Rdata
```{r}
...
# Process Data
*Must begin here to continue analysis below.
*Loads data_states.Rdata. 
*This dataframe contains ODBA values and classifications identified by fitting a 4-state HMM.
```{r}
...
### Calculate state proportions for day and night.
*KJH: Add this from below.
...
*KJH: This requires stateSummaryIND2period2state created below. Must create above.
...
*Creates sumODBA, sumODBASU, and sumODBAWI
*KJH: Need to convert ODBA from ms-2 to g (divide by 9.81)
...
# Create dataframe of seasonal ODBA totals
sumODBASU <- sumODBA %>% 
...
*Also confirms no effects from tag versioning (by default, if no interannual difference by season).
Results: There is no difference in total ODBA per season between years (summer p=0.83, winter p=0.11).
...
*KJH: These values are in ms-2. Need to convert in sumODBA calculation above.
*Results: Daily ODBA values are higher during winter than in summer (winter = 27939.53, summer = 23356.13, p-value = 0.03567).
...
*KJH: Add caption (results)
*KJH: Needs to add shading for day night 
*KJH: Change x-axis ticks 0-24 by 4
*KJH: Add a caption to the figure?
...
# Save a file at 300 ppi
# ggsave(Podba24Seas, file="Seasonal total ODBA per solar hour_black.png",width=12, height=6, dpi=300)
```


## Q2a: Is there a seasonal difference in daytime absolute duration of the active state?

### Calculate durations and proportions
...
### Statistical test of seasonal differences in daytime states
*Uses dataframes dayActive and dayRest created above
*Results: I found no seasonal difference in daytime absolute duration of the active state (winter = 5.48 ± 2.18 hr, summer = 5.19 ± 2.38 hr (mean ± SD). The additional summer day length is spent resting (winter = 5.34 ± 2.20 hr, summer = 11.56 ± 2.06 hr (mean ± SD), Two Sample t-test p = 3.61e-07).
...
*Creates plot called "Seasonal daytime absolute state durations of active and rest"
*KJH: Can I use dayActive and dayRest instead of stateSummaryIND2period2state?
```{r}
...
*This plots rest and active by solar hour. During summer, many resources are available across time. Whereas during winter, resources are spatiotemporally fixed.
