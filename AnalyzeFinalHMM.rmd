...
stateBout$activity <- ifelse(stateBout$state_classif==1|stateBout$state_classif==2,
                             "Rest","Active")
...
stateBoutSum <- stateBout %>%
  group_by(season,ID,boutNum,state_classif,dayNight,hr,activity) %>% 
  summarize(boutLength=first(boutLength)) %>%
  ungroup
```

## Plot activity durations by hour to look at patterns
```{r}
# Plot summer daytime activity absolute duration by hour
PdurSUd <- stateBoutSum %>% 
  group_by(season,ID,dayNight,hr,activity) %>%
  mutate(boutLengthDur=sum(boutLength)) %>%
  filter(season=="Summer",
         dayNight=="day") %>% #if want to filter, add state_classif %in% c(3,4)
  ggplot() +
  geom_boxplot(aes(x = hr, y = boutLengthDur, 
                   color = activity)) +
  labs(x="Hour of Day",
       y = "Duration (min)",
       title = "Summer daytime activity absolute duration by hour") + 
  theme_classic(base_size = 13) + 
  theme(plot.title = element_text(size=14)) +
  theme(axis.text = element_text(size = 14, color = "black"),
        axis.title.x = element_text(color="black", size=14),
...
PdurWId <- stateBoutSum %>% 
  ...
  labs(x="Hour of Day",
       y = "Duration (min)",
       title = "Winter daytime activity absolute duration by hour") + 
  theme_classic(base_size = 13) + 
  theme(plot.title = element_text(size=14)) +
  theme(axis.text = element_text(size = 14, color = "black"),
        axis.title.x = element_text(color="black", size=14),
        axis.title.y = element_text(color="black", size=14))
# PdurWId
PdurSeasd <- ggarrange(PdurSUd,PdurWId,nrow=2)
PdurSeasd

# Plot summer nighttime activity absolute duration by hour
PdurSUn <- stateBoutSum %>% 
  group_by(season,ID,dayNight,hr,activity) %>%
  mutate(boutLengthDur=sum(boutLength)) %>%
  filter(season=="Summer",
  ...
  labs(x="Hour of Day",
       y = "Duration (min)",
       title = "Summer nighttime activity absolute duration by hour") + 
  theme_classic(base_size = 13) + 
  theme(plot.title = element_text(size=14)) +
  theme(axis.text = element_text(size = 14, color = "black"),
        axis.title.x = element_text(color="black", size=14),
...
PdurWIn <- stateBoutSum %>% 
  ...
  labs(x="Hour of Day",
       y = "Duration (min)",
       title = "Winter nighttime activity absolute duration by hour") + 
  theme_classic(base_size = 13) + 
  theme(plot.title = element_text(size=14)) +
  theme(axis.text = element_text(size = 14, color = "black"),
        axis.title.x = element_text(color="black", size=14),
        axis.title.y = element_text(color="black", size=14))
# PdurWIn
PdurSeasn <- ggarrange(PdurSUn,PdurWIn,nrow=2)
PdurSeasn
```
...
PboutSUd <- stateBoutSum %>% 
  group_by(season,ID,dayNight,activity,hr) %>%
