...
# Test for normality
shapiro.test(odbaRated$odbaGrate[odbaRated$season=="Summer"])
shapiro.test(odbaRated$odbaGrate[odbaRated$season=="Winter"])

# Test for equal variances
var.test(odbaGrate ~ season,data=odbaRated) #Variance is unequal p=0.03201
...
### Q2: Confirm no difference in Amag between 2017 and 2018/2019
...
test
sd(diff$diff[diff$ver=="V2"],na.rm=TRUE)
```
...
PdayDur2state <- stateSummaryIND2period2state %>%
  ...
  scale_x_discrete(labels = c("Active", "Rest")) +
  labs(x="State",
       y = "Duration (hr)",
...
PdayDur4state <- stateSummaryIND2period %>%
  ...
  labs(x="State",
       y = "Duration (hr)",
       title = "Seasonal daytime amount of activity by degree") + 
  theme_classic(base_size = 13) + 
  theme(legend.position = "none") +
  theme(plot.title = element_text(size=14)) +
  theme(axis.text = element_text(size = 14, color = "black"),
...
# Test for normality
shapiro.test(dayODBAstate4$rateODBAstateHr[dayODBAstate4$season=="Summer"]) #Results: p-value =
shapiro.test(dayODBAstate4$rateODBAstateHr[dayODBAstate4$season=="Winter"]) #Results: p-value = 

# Test for equal variance
var.test(rateODBAstateHr ~ season, data = dayODBAstate4)

# Test for seasonal differences in daytime active state
t.test(rateODBAstateHr ~ season, data = dayODBAstate4, var.equal = TRUE) #Results: p-value = 
...
```{r}
PactiveDiff <- ggarrange(PdayDur2state,PdayDur4state,PdayODBAstate, nrow=3)
...
#### Calculate seasonal mean daylength and nightlength
This needs work.

```{r}
# KJH: Need to convert data_states$dawnT and duskT to as.POSIXct
# Calculate mean dawn and dusk per season
mDawnDusk <- data_states %>% 
  summarize(meanDawnSU=as.Posixct(mean(unique(data_states$dawnT[data_states$season == "Summer"])),
            meanDuskSU=mean(unique(data_states$duskT[data_states$season == "Summer"])),
            meanDawnWI=mean(unique(data_states$dawnT[data_states$season == "Winter"])),
            meanDuskWI=mean(unique(data_states$duskT[data_states$season == "Winter"]))))
              

```

#### Plot daytime duration states 3 and 4 by season
...
#### State Durations per Individual (Night)
```{r}
