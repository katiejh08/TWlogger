...
```{r}
# Calculate state proportions (4 states) with 2 periods (day and night)
...
Pday2
# Save a file at 300 ppi
ggsave(Pday2, file="Seasonal Daytime State Proportions.png",width=12, height=6, dpi=300)
# Plot night
...
PdayDur <- stateSummaryIND2period %>%# View
  ...
  labs(x="State",
       y = "Duration (hr)",
       title = "Seasonal daytime state durations per individual (dawn to dusk)") + 
  ...
  theme(axis.text = element_text(size = 14, color = "black"),
        ...
        axis.title.y = element_text(color="black", size=14))
PdayDur
ggsave(PdayDur, file="Seasonal daytime absolute 4-state durations.png",width=8, height=8, dpi=300)
...
### Plot Absolute durations by combined state (active and rest) by period (day + night)
*Need to adapt this script
```{r}
...
PstateDurSU <- stateDur2period %>% 
  ...
  labs(x="Period",
       y = "Duration (hr)",
       title = "Austral summer absolute state durations per individual") + 
  theme_classic(base_size = 13) + 
  ...
  theme(axis.text = element_text(size = 14, color = "black"),
        ...
        axis.title.y = element_text(color="black", size=14))
PstateDurSU
# Plot summer states (active and rest) by combined period (day and night)
PstateDurWI <- stateDur2period %>% 
  ...
  labs(x="Period",
       y = "Duration (hr)",
       title = "Austral winter absolute state durations per individual") + 
  theme_classic(base_size = 13) +
  scale_color_discrete(name = "State") +
  scale_y_continuous(breaks = seq(0, 24, by = 6)) +
  theme(plot.title = element_text(size=14)) +
...
# Save a file at 300 ppi
ggsave(PstateDur24hr, file = "Seasonal absolute state durations by period.png",width=8, height=8, dpi=300)

PstateDur <- stateDur2period %>% 
  filter(dayNight=="day") %>% 
  ggplot() +
...
PstateDur
# Save a file at 300 ppi
ggsave(PstateDur, file="Seasonal daytime absolute state durations.png",width=8, height=8, dpi=300)
```
...
## ODBA
## Statistical test of differences in total odba per year to confirm pooling
```{r}
