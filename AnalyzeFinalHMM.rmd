...
I use HMM state classifications to parse the meaning of my ODBA results. Pooling the resting (states 1 & 2) and active (states 3 & 4) states masks the seasonal difference in acivity levels. When analyzed as 4-states, the The HMM gives higher resolution than necessary to ask the question regarding active vs. rest. However, 
...
#### Test for seasonal differences in hourly ODBA (g)
...
#### Plot daytime ODBA normalized by day length (daily ODBA rate)
...
stateBout <- data_states %>% 
  ...
  mutate(boutLengthSec = n(), #bout length is in seconds
         boutODBAsum = sum(ODBA),
         boutODBAmean = mean(ODBA)) %>% #mean gives an index of energy use per state 
  ungroup
