...
daytimeState4 <- stateSummaryIND2period %>% 
  ...
  filter(dayNight=="day",
         state_classif=="4") %>%
  ungroup
daytimeState4meanSU <- mean(daytimeState4$stateCountHour[daytimeState4$season == "Summer"])
daytimeState4meanSU
daytimeState4SDSU <- sd(daytimeState4$stateCountHour[daytimeState4$season == "Summer"])
daytimeState4SDSU

daytimeState4meanWI <- mean(daytimeState4$stateCountHour[daytimeState4$season == "Winter"])
daytimeState4SDWI <- sd(daytimeState4$stateCountHour[daytimeState4$season == "Winter"])
daytimeState4meanWI
daytimeState4SDWI
...
### Plot histogram of state 3 and state 4
```{r}
data_states$state_classif <- as.factor(data_states$state_classif)
# State 4
dist_boutLength <- stateBoutSum %>% 
  group_by(season,state_classif) %>%
  ggplot()+
  geom_histogram(aes(boutLength))+
  facet_grid(season~state_classif)
dist_boutLength
ggsave(dist_boutLength, file="ODBA distributions grouped by season and state.png",width=12, height=6, dpi=300)
