...
# Pre-Process Data
*Pre-processing scripts import and combine data frames to produce data_states.Rdata.
*Only need to run this section once, unless needing to recreate data_states.Rdata.
...
*This creates one file with states and raw acc. Saves as data_states.Rdata, so do not need to run this pre process more than once.
```{r}
...
# Process Data
*Loads data_states.Rdata. Must begin here to continue analysis below.
```{r}
...
## States
*Requires data_states.Rdata.
*These use state classifications identified by fitting a 4-state HMM.

### Calculate state proportions for each astronomical period (dawn, day, dusk, night). Will calculate by day and night below.
*Calculates count of each state per period. Calculates state proportion by dividing by period length.
*Creates stateSummaryIND. This is used to calculate day and night active and rest states below. Can combine scripts at a later date.
```{r}
...
*Statistical test for seasonal difference in nighttime state proportions.
*Plots nighttime absolute state durations and proportions.
*Results: (VISUAL) There is no seasonal difference in proportion of nighttime spent in either the active or resting state. 
...
### Confirm pooling seasons among years.
*Also confirms no affects from tag versioning.
*Statistical test of differences in total ODBA per year.
Results: There is no difference in total ODBA per season between years (summer p=0.82, winter 0.11).
```{r}
...
### Test if tag versioning (2017 vs 2018/19) affected ODBA.
*Statistical test of seasonal 
Results: There is no seasonal difference in total odba per solar hour 
```{r}
sumODBASU <- sumODBA %>% 
  group_by(ID,season,yr) %>%
  filter(season == "Summer") 
t.test(sumODBA ~ yr, data=sumODBASU)
...
Podba24Seas 
# Save a file at 300 ppi
ggsave(Podba24Seasblack, file="Seasonal total ODBA per solar hour_black.png",width=12, height=6, dpi=300)
```
  
### Statistical test of seasonal differences in total ODBA
*Calculates total daily ODBA per individual
*Creates sumODBA
*Statistical test of differences in seasonal total ODBA per solar hour
```{r}
# Check for NAs
sapply(data_states, function(x) sum(is.na(x)))

# Calculate sum of ODBA per day per ID
sumODBA <- data_states %>% 
  group_by(season,ID) %>%
  summarize(sumODBA = sum(ODBA,na.rm = TRUE))

# Create dataframe of seasonal ODBA totals (to use with density plots)
sumODBASU <- sumODBA %>% 
  filter(season == "Summer")
sumODBAWI <- sumODBA %>% 
  filter(season == "Winter") 
...
# STATISTICAL TEST
test <- t.test(sumODBA$sumODBA)
test
```

### ODBA general linear mixed model
*Runs a glmer with response as 1-Hz ODBA, interaction terms of season and state, and random effects of individual and year.
*KJH: This needs work. Need to continually resample until coefficients converge.
```{r}
# General linear mixed model (glmer) with response as 1-Hz ODBA, interaction terms of season and state, and random effects of individual and year
m1 <- glmer(ODBA~season*state_classif + (1|ID) + (1|yr),
            data=data_states,
            family=poisson)
m1
```
...
*Calculate 4-state duration and proportion for 2 periods (day and night)
*Calculate daytime 2-state (active and rest) duration and proportion for 2 periods (day and night)
*Creates stateSummaryIND2period
*Creates stateSummaryIND2period2state
*Creates dayActive & dayRest
*Statistical test of seasonal differences in daytime active state
 
*Results: I found no seasonal difference in daytime duration of the active state (winter = 5.48 ± 2.18 hr, summer = 5.19 ± 2.38 hr (mean ± SD). The additional summer day length is spent resting (winter = 5.34 ± 2.20 hr, summer = 11.56 ± 2.06 hr (mean ± SD), Two Sample t-test p = 3.61e-07).
...
# STATISTICAL TEST
# Seasonal differences in daytime active state
...
t.test(stateCountHour ~ season, data = dayActive, var.equal = TRUE) #Results: p-value = 0.7531
sd(dayActive$stateCountHour[dayActive$season == "Summer"])
sd(dayActive$stateCountHour[dayActive$season == "Winter"])

# Seasonal differences in daytime rest state
...
*Creates "Seasonal daytime absolute state durations of active and rest"
```{r}
...
```{r}
# Plot summer daytime activity absolute duration by solar hour
...
## Q2b: Is there a seasonal difference in daytime absolute duration of state 3? What about a seasonal difference in daytime absolute duration of state 4?
*Statistical test of seasonal differences in daytime absolute durations of state 3 and state 4
*Results: Within the daytime active state, I found no seasonal difference in time spent in state 3 (winter = 3.83 ± 1.62 hr, summer = 4.45 ± 2.01 hr (mean ± SD)), however, during winter birds spend more time in state 4 (winter = 1.65 ± 0.75 hr, summer = 0.74 ± 0.44 hr (mean ± SD), Welch's Two Sample t-test p < 0.01). See Figure 04b.

```{r}
# STATISTICAL TEST
# Seasonal differences in daytime state 3
...
shapiro.test(state3day$stateCountHour[state3day$season=="Summer"]) #Results: p-value = 0.0476
shapiro.test(state3day$stateCountHour[state3day$season=="Winter"]) #Results: p-value = 0.2116
t.test(stateCountHour ~ season, data = state3day, var.equal = TRUE) #Results: p-value = 0.4173
wilcox.test(stateCountHour ~ season, data = state3day, var.equal = TRUE) #Results: W = 86, p-value = 0.4428
state3daySUsd <- sd(state3day$stateCountHour[state3day$season=="Summer"])
state3daySUsd
state3dayWIsd <- sd(state3day$stateCountHour[state3day$season=="Winter"])
state3dayWIsd

# STATISTICAL TEST 
# Seasonal differences in daytime state 4
...
shapiro.test(state4day$stateCountHour[state4day$season=="Summer"]) #Results: p-value = 0.2107
shapiro.test(state4day$stateCountHour[state4day$season=="Winter"]) #Results: p-value = 0.2817
t.test(stateCountHour ~ season, data = state4day, var.equal = TRUE) #Results: p-value = 0.001439
wilcox.test(stateCountHour ~ season, data = state4day, var.equal = TRUE) #Results: p-value = 0.001433
sd(state4day$stateCountHour[state4day$season == "Summer"])
sd(state4day$stateCountHour[state4day$season == "Winter"])

state3day
state4day
```

### Plot of seasonal daytime absolute durations per state
```{r}
# Plot daytime absolute state durations
...
*Calculates state bout length (seconds), combined states (active and rest), and combined periods (day and night)
*Calculates timeBin vector based off solar hour of day
*Creates stateBout & stateBoutSummary
*Creates  state3boutDayIndMeans (seconds) and state4boutDayIndMeans
*Statistical test of seasonal differences in mean bout lengths of state 3 and state 4
*Need to use test for samples of unequal lengths (summer days are longer)
...
*Creates plot of seasonal mean state bouts (active and rest) by solar hour. 
...
### Plot histograms of bout lengths (minutes) by state and season
*Creates "Distribution of daytime state bouts by austral season"
*Creates "Distribution of daytime state 4 bouts per bird by austral season"
*Creates "Distribution of daytime state 3 bouts per bird by austral season"
...
### Plot distribution of states by season
*This shows data is left skewed
```{r}
...
*KJH: This needs work.
```{r}
# Summer mean ODBA per solar hour by season
...
# Winter mean ODBA per solar hour by season
