...
### Calculate state proportions for each astronomical period (dawn, day, dusk, night)
...
### Is there a seasonal difference in daytime absolute duration of the active state?
...
rm(dayActive,dayRest)
```


### Is there a seasonal difference in daytime absolute duration of state 3? What about a seasonal difference in daytime absolute duration of state 4?
...
state3dayWIsd
rm(state3daySUsd,state3dayWIsd)
...
# Still need script to pull results into table
```

### Is there a seasonal difference in mean bout lengths? By day? By night? By 24-hr cycle?
*Calculates state bout length (minutes), combined states (active and rest), and combined periods (day and night)
...
stateBout <- data_states %>% 
  ...
  mutate(boutLength = n()) %>% #bout length is in seconds
  ungroup
...
hist(stateBoutSummary$boutLength[stateBoutSummary$season=="Summer" & 
...
state3boutDayIndMeans <- stateBoutSummary %>% 
  group_by(season,ID,dayNight,state_classif,boutLength) %>%
  filter(dayNight=="day",
         state_classif=="3") %>%
  ungroup %>% 
  group_by(ID,season) %>%
  summarize(meanBoutLength=mean(boutLength))
```

### Plot 4-state absolute durations by period (day and night)
*Creates "Seasonal daytime absolute 4-state durations" and "Seasonal State Durations by Two Periods"
...
PodbaSum24Wiblack <- data_states %>%
  ...
  theme(axis.text = element_text(size = 14, color = "black"),
        ...
        axis.title.y = element_text(color="black", size=14))
PodbaSum24Wiblack
Podba24Seasblack <- ggarrange(PodbaSum24Sublack,PodbaSum24Wiblack,nrow=2)
Podba24Seasblack
# Save a file at 300 ppi
...
PodbaMeanWiblack <- data_states %>%
  ...
  theme(axis.text = element_text(size = 14, color = "black"),
        ...
        axis.title.y = element_text(color="black", size=14))
# PodbaMeanWi
PodbaMeanSeasblack <- ggarrange(PodbaMeanSublack,PodbaMeanWiblack,nrow=2)
PodbaMeanSeasblack
# Save a file at 300 ppi
...
### Statistical test of seasonal differences in total odba per solar hour by season
```{r}
# Combined years
# Check for NAs
sapply(data_states, function(x) sum(is.na(x)))
...
sumODBAWI <- sumODBA %>% 
  filter(season == "Winter") 
length(sumODBASU$sumODBA)
length(sumODBAWI$sumODBA)
...
### Statistical test of seasonal interannual differences in total odba per solar hour by season
*This tests if tag versioning (2017 vs 2018/19) affected ODBA.
```{r}
sumODBASU <- sumODBA %>% 
...
### Alternative plot to Figure 1
*Not using this
*This creates a boxplot of seasonal oDBA totals per solar hour
```{r}
