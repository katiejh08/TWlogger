...
```{r}
# Calculate with 4 periods (dawn, day, dusk, night)
...
# Reduce to two periods day and night (e.g. combine dawn, day, and dusk)
...
```{r}
# Plot combined day
PdayDur <- stateSummaryIND2period %>% 
  group_by(season,dayNight) %>% 
  filter(dayNight == "day") %>% 
  ggplot() +
...
PdayDur
# Plot night
PnightDur <- stateSummaryIND2period %>% 
  group_by(season,dayNight) %>% 
  filter(dayNight == "night") %>% 
  ggplot() +
  geom_boxplot(aes(x = state_classif, y = stateCountHour, 
...
# Save a file at 300 ppi
ggsave(PcycleDur, file="Seasonal State Durations by Two Periods.png",width=12, height=6, dpi=300)
```
...
# Statistical test to check for difference in state duration between summer and winter days
...
# seasdiffdur <- cbind(seasdiffdur1,seasdiffdur2) %>% 
#   subset(.,select = c(season,dayNight,state_classif,stateCountHourWI,stateCountHourSU))
...
```{r}
# Add ODBA to data
datax <- data # ODBA contained in this df at 1Hz
datax <- subset(datax, select = -c(Ax,Ay,Az,Amag,Amag_rollmean_10,Amag_rollmean,state_classif,pitch,roll))
stateODBA <- left_join(data,dplyr::select(datax,ID, dttz, odba, jerk), by = c("ID", "dttz"))
stateODBA <- stateODBA %>% 
  group_by(season, ID) %>% 
  mutate(dayNight = ifelse(astronomical %in% c("dawn","day","dusk"),"day","night")) %>%
  ungroup
