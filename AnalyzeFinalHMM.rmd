...
# Pre-Process Data
Pre-processing code imports and combines data frames to produce data_states.Rdata. Only need to run this section once, unless needing to recreate data_states.Rdata.
...
This plots nighttime absolute state durations and proportions. Results suggest there is no seasonal difference in proportion of nighttime spent in either the active or resting state. 
...
### Statistical test of seasonal differences in total daily ODBA (g)
...
shadeSU = data.frame(x1=c(0,mean(hour(data_states$dawn[data_states$season == "Summer"]))), 
                     x2=c(mean(hour(data_states$dusk[data_states$season == "Summer"])),24), 
                     y1=c(0,0), 
                     y2=c(800,800))
shadeWI = data.frame(x1=c(0,mean(hour(data_states$dawn[data_states$season == "Winter"]))), 
                     x2=c(mean(hour(data_states$dusk[data_states$season == "Winter"])),24), 
                     y1=c(0,0), 
                     y2=c(800,800))
...
PodbaSum24SU <- data_states %>%
  ...
  ylim(0,800) +
  # xlim(0,24 )+
  # scale_x_time(breaks = "2 hours", labels = "%H") +
  # scale_x_discrete(breaks = seq(0,24, by = 2)) +
  labs(x=NULL,
       y = NULL,
       title = "(a)") + 
  geom_rect(data=shadeSU, 
            mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), color='grey', alpha=0.5) +
  theme_classic(base_size = 13) +
  theme(plot.title = element_text(size=14)) +
  theme(axis.text = element_text(size = 14, color = "black"),
        axis.title.x = element_text(color="black", size=14),
...
PodbaSum24WI <- data_states %>%
  ...
  labs(x="Solar Hour",
       y = NULL,
       title = "(b)") + 
  geom_rect(data=shadeWI, 
            mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), color='grey', alpha=0.5) +
  # scale_x_discrete(data_states$timeBin) +
  theme_classic(base_size = 13) + 
  theme(plot.title = element_text(size=14)) +
  theme(axis.text = element_text(size = 14, color = "black"),
        axis.title.x = element_text(color="black", size=14),
...
Podba24Seas 
annotate_figure(Podba24Seas,
               top = text_grob("Seasonal total ODBA by solar hour", color = "black", size = 14),
               bottom = text_grob("Figure 1. Daily ODBA values are higher during winter than in summer (winter 2848.07 ± 577.26 g (mean ± SD), summer 2380.85 ± 435.65 g, p-value = 0.03567).", color = "black",
                                  hjust = 1, x = 1, size = 10),
...
This calculates:
State bout length (seconds), combined states (active and rest), and combined periods (day and night)
