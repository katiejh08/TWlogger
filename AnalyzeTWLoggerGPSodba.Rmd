...
```{r Libraries,echo=FALSE,results="hide",warning=FALSE,collapse=TRUE,message=FALSE}
library(tidyverse)
library(dplyr)
...
for(i in 1:length(filenames)) {
  ...
  startTime <- depTimes %>% dplyr::filter(ID == name) %>% .$startTime
  endTime <- depTimes %>% dplyr::filter(ID == name) %>% .$endTime
