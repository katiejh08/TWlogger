...
for(i in 1:length(depMeta$depid)) {
  ...
  # Save as .RData file
  save(data, file=paste0(depid,"-10Hz-VDBA-ODBA.RData"))
  
  # Down sample to 1Hz ------------------------------------------------------
  data <- data %>% 
    dplyr::mutate(secs_since = floor(as.numeric(dttz - min(dttz)))) # seconds since start of deployment
  # Calculate mean VDBA and ODBA (down sample from 50 to 1Hz)
  data <- data %>% 
    group_by(secs_since) %>%
    summarize(dttz=first(dttz),
              Ax=mean(Ax),
  ...
  # Save as .RData file
  save(data, file=paste0(depid,"-1Hz-VDBA-ODBA.RData"))
  setwd(current_path)
  rm(current_path)
}
