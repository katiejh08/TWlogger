...
GPS_states already has x, y, lat, and long in the dataframe.
...
PkdeSU <- ggmap(myMapSU) + #ggplot() + 
  ...
  # stat_density2d(aes(x = long, y = lat, group=ID, fill = ..level.., alpha = ..level..),
...
PkdeWI <- ggmap(myMapWIkde) + #ggplot() + 
  ...
  annotate("text", x = -59.95, y = -51.22, label = "b", color="white") +
  coord_fixed(1.3) +
  geom_rect(aes(xmin = -60.255, xmax = -60.22, ymin = -51.32, ymax = -51.29),
  alpha = .5,fill=NA,linetype="solid",color="white") +
  theme(legend.position = "none") +
...
PkdeSeas <- ggarrange(PkdeSU,PkdeWI,ncol=2)
PkdeSeas
ggsave(PkdeSeas, file="Seasonal kde_white and Neck box.png",width=12, height=8, dpi=300)
...
save(GPS_states,file="GPS_states_190617.RData")
```
